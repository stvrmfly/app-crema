// Centralized SVG icons (Heroicons outline, 24x24 viewBox).
// All icons accept the standard SVG props (className, strokeWidth, etc).
// Default size: w-5 h-5. Override with className.
//
// Why one file: avoids duplicate path strings across pages, gives us one place
// to swap icon sets later (Lucide, Phosphor, etc.) without grep-and-replace.

const base = 'inline-block flex-shrink-0';

function Icon({ children, className = 'w-5 h-5', strokeWidth = 1.5, ...rest }) {
  return (
    <svg
...
export const BanknoteIcon = (props) => (
  // Expenses sidebar entry — Heroicons "banknotes" outline
  <Icon {...props}>
...
// Spinning ring for inline loading states. Pair with `animate-spin` from Tailwind.
export const SpinnerIcon = ({ className = 'w-4 h-4', ...rest }) => (
  <svg
    className={`${className} animate-spin inline-block flex-shrink-0`}
    viewBox="0 0 24 24"
    fill="none"
    aria-hidden="true"
    focusable="false"
