...
function LoginPage() {
  ...
  const entering = location.state?.fromApp === true
  ...
  function enterApp() {
    setExiting(true)
    // Panel falls at 0ms, title falls at 120ms — both 380ms. Wait for title to finish.
    setTimeout(() => navigate('/app', { state: { fromLanding: true } }), 500)
  }
  ...
  return (
    <>
    <AuthLayout exiting={exiting} entering={entering}>
    ...
    {/* Dev tools — fixed to bottom-left of viewport (outside AuthLayout to escape
        transformed ancestors). Gated to dev builds so a real customer never sees
        "Skip login" / "Fill form". */}
    {import.meta.env.DEV && (
